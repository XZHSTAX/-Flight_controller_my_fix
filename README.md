...
7.28-2：**Connect to OpenMV2-add our heigh control**。加入了我们自己的高度控制，openMV只发送`S`，即悬停信号。

7.28-3：**Connect to OpenMV3-Can't fix the problem of shake**注释了OpenMV控制中有关`dy_height`的内容，使得高度完全由飞控控制。把发送`H`开始OpenMV控制从延时发送，修改为达到指定高度才发送，即到达指定高度，OpenMV开始控制。
...
7.29-3: **Stable with help of OP** 做出了以下调整使得仅仅光流作用下无人机可以较为平稳的悬停。首先，只是用了一个高度环，当系统的高度环和phs的共同作用时，就会出现抖动的情况。更换了地贴，现在使用地贴是之前小车留下的纯白布。此外这套代码也实现了和OpenMV的联动。现在高度环是phs写的。
